...
## 2026-09-18 持续 HTTP 406 的恢复策略

- HTTP 406 等不可重试的客户端错误不再原样重试。API 失败时读取 arXiv 官方 `https://rss.arxiv.org/atom/<category>` 每日公告；API 和公告都失败才退回纯缓存。全部分类的两个来源都失败时仍阻止发布。
- 公告模式明确提示历史缺口待补齐。卡片标记公告日期，不把它伪装成论文首发或修订日期。
- `last_api_success` 单独记录 API 查询进度。查询从最后一次 API 成功前两天开始，保留重叠窗口；无缓存时补取完整保留窗口。公告成功不会推进 API 进度。
- 同一论文优先最新版本，同版本优先 API 的精确元数据。
- GitHub runner 同时验证实际 API 构建与四个官方公告源。备用源也可能暂时不可用，不能保证外部服务永远成功。
